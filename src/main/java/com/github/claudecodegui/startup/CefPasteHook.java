...
public class CefPasteHook extends CefKeyboardHandlerAdapter {
    ...
    public static void install(CefClient client, ClaudeChatWindow window) {
        try {
            if (client == null) { LOG.warn("[paste-fix][CEF] client null"); return; }
    ...
    public boolean onPreKeyEvent(CefBrowser browser, org.cef.handler.CefKeyboardHandler.CefKeyEvent event, BoolRef isKeyboardShortcut) {
        try {
            if (event.windows_key_code != VK_V) return false;
            ...
            if (event.type != org.cef.handler.CefKeyboardHandler.CefKeyEvent.EventType.KEYEVENT_KEYUP) return false;
            ...
            if (now - lastPasteTrigger < PASTE_DEBOUNCE_MS) return false;
            ...
            if (window == null) { LOG.info("[paste-fix][CEF] no chat window"); return false; }
            SwingUtilities.invokeLater(() -> handlePaste(window));
        } catch (Throwable t) {
