...
**日期**: 2026-08-26  
**状态**: 阶段 A 已实现；已按倒序展示的视觉调序语义修正，阶段 B/C 待执行  
...
| 阶段 C：高风险调度 | 8、9（相关部分）、10 | 支持打断当前任务并优先安排目标队列项 | 待开始 | 高；与中断、loading 变化和自动消费队首的时机耦合 |
...
- [ ] 实现 `interruptAndSendNow`，复用 `interruptSession` 并确保目标项成为逻辑队首。
- [ ] 覆盖 loading 状态切换、自动消费与中断竞态的 Hook 测试。
- [ ] 增加打断按钮的状态、失败反馈和无障碍交互测试。
- [ ] 完成中断流程与队列自动消费的手动回归。
- [ ] 验证完成后开启 `interrupt` 功能开关。
